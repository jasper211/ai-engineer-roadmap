...
def parse_args():
    ...
    parser.add_argument("--build-scenario-analysis", action="store_true", help="调用AI模型为每个业务场景推理目标/流程现状/数据治理/任务清单/流程优化，MODEL_DRAFT标注")
...
def main() -> int:
    ...
    if args.sync_business_scenarios:
        ...
        return 0
    if args.build_scenario_analysis:
        from skills.business_scenario_analysis import run_scenario_analysis, write_scenario_analysis

        scenario_dest_dir = AGENT_ROOT / "10_部署与运行_Deploy_and_Run/frontend/public/data/business_scenarios"
        index = json.loads((scenario_dest_dir / "index.json").read_text(encoding="utf-8"))
        ...
        for entry in index["scenarios"]:
            ...
            result = run_scenario_analysis(AGENT_ROOT, scenario, model_index, db_catalog)
            write_scenario_analysis(result, analysis_dest_dir / f"{scenario['scenario_id']}.json")
            analyzed_scenarios.append(scenario["scenario_id"])

        print(json.dumps({"status": "published", "analyzed_scenarios": analyzed_scenarios}, ensure_ascii=False, indent=2))
        return 0
