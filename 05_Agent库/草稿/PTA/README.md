...
`PTA-DASH`/`EXT`（批1）、`PTA-DISCOVER`/`SCAN`（批2）已迁移进 skills/tools，
详见 `11_监控与优化_Monitor_and_Optimize/README.md` 的迁移进度记录。

`PTA-INTEL`/`INTEL-RW`（批3，两份高度重复的实现，待合并成一个技能）暂未
迁移，仍以独立扁平脚本形式存在，按方法论归到 `11_监控与优化_Monitor_and_Optimize/` 下。
