...
界面采用浅色工作台主题，以白色信息卡片、浅灰背景和紫色重点标识为主。

首页以三个项目最新成功巡检之间的完整文件变化为SSOT：EA流程架构项目是核心
主视图，Jasper工作文档是AI技术试验田，Rw权益项目是真实案例观察窗口。任务、
执行建议和跨项目关系都是文件事实的下游分析，不再反客为主。

`P&L_EA小组Jasper` 已作为 EA 小组经营分析项目接入，复用 EA 的流程/SOP、
人机规则、Agent化与责任归口分析标准，并进入同一企业微信每日通知链路。
...
- 指挥中心筛选：支持按新增/修改/删除、项目成员、文件名/路径/摘要组合筛选；
  项目计数、相关任务、项目关系和跨项目线索随筛选范围收敛，并即时生成筛选结果
  的领域、成员、删除风险与重点更新速读分析。
- 历史范围：支持聚合过往 30/7/3/1 天的落盘巡检报告；日期以报告生成时间为
  准，同一文件跨日多次变化分别保留，并在文件行显示发现时间。
- 重要文件：与“与你相关”共用同一判断结果。EA直接行动及已确认的Jasper→EA
  应用事项所引用的文件自动置顶，以黄色边框标注，并展示关联理由与任务ID。
