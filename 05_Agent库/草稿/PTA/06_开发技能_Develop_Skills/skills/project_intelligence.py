...
CrossDocumentAnalyzer），但翻开数据模型会发现根本不是一回事：
  ...
  - PTA-INTEL-RW（Rw 项目专用版）：精确读 Rw 项目固定几份跟踪台账 CSV 的
    ...
    探测目标项目目录下有没有 Rw 特征 CSV（RwProjectParser.TRACKING_FILES
    列的那几份固定文件名），有就用 Rw 专用解析器，没有就退回通用解析器。
    对调用方（agent.py）来说只有一个入口，不需要自己判断该用哪个。
...
class RwTrackItem:
    ...
    def is_blocked(self) -> bool:
        return (any(kw in self.current_status.lower() for kw in ["blocked", "red", "阻塞", "停止"])
                or bool(self.blocker and self.blocker != "无"))
...
def is_rw_project(project_root: Path) -> bool:
    ...
    return any((project_root / f).exists() for f in RwProjectParser.TRACKING_FILES)


class ProjectIntelligence:
    """统一入口：自动探测项目目录下有没有 Rw 特征 CSV，选通用解析器还是
    Rw 专用解析器，调用方不需要自己判断该用哪个后端。"""

    def __init__(self, project_root: Path):
        self.project_root = Path(project_root)
        self.is_rw = is_rw_project(self.project_root)
        if self.is_rw:
            tracks, charter, reports = RwProjectParser(self.project_root).parse_all()
