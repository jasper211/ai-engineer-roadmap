...
看到全是 ✅ 就是成功了（截至本文档更新时共 19 项，覆盖 5 个核心技能 +
agent.py 全链路 + 跨项目知识库 + git 同步安全行为验证 + 每日巡检/企业微信
通知/docx-xlsx 解析/规则扫描/文档任务发现等扩展能力）。

### 4.（可选）如果要用需要 LLM 的功能（--daily-scan / --discover）
...
**文档任务发现（--discover）已并入 agent.py**（批2迁移）：

```bash
# 增量扫描项目内所有候选文档（.md/.txt/.csv/.docx/.xlsx），需要 DEEPSEEK_API_KEY
python3 04_定义Agent_Define_Agent/agents/agent.py --discover --scan --project-root /path/to/project

# 只看候选文件和估算字符数，不实际调用 API
python3 04_定义Agent_Define_Agent/agents/agent.py --discover --scan --dry-run --project-root /path/to/project
```

产出只是供人工审阅的发现记录（进目标项目专属工作区的 `task_registry.json`），
不会自动写入 `pta_tasks.json` 的可执行流程。
...
## 已并入 agent.py 的扩展能力 + 仍是独立脚本的部分

| 能力 | 命令 |
|------|------|
| DASH 项目仪表盘（批1） | `agent.py --dashboard --project-root /path --person <name>` |
...
| 依赖 | 版本 | 说明 |
|------|------|------|
| Python | 3.9+ | `Path.is_relative_to` 等用法需要 3.9+ |
| Git | 任意 | doc_sync / agent.py --sync 需要 |
| DeepSeek API Key | — | 只有用 `--daily-scan`/`--discover` 时需要，`export DEEPSEEK_API_KEY=sk-xxx` |
| python-docx / openpyxl | 见 `requirements.txt` | 只有需要解析 .docx/.xlsx 内容时才用到（daily_sensing、document_task_discovery），缺失时会优雅降级为"仅感知到文件变化，无法解析内容"，不影响其余功能 |
...
| `Module not found` | 核心功能不需要安装任何包（python-docx/openpyxl 是唯二例外，见上表）；LLM 相关功能遇到 SSL 证书报错见下一条 |
| `--daily-scan`/`--discover` 报 SSL 证书错误 | 已知的 Homebrew Python 证书路径问题，`tools/llm_client.py` 的 `build_ssl_context()` 内置了 certifi/系统证书兜底，正常应该不会再出现 |
