...
# -*- coding: utf-8 -*-
"""
工具：LLM 调用封装（原样复制自 OB 的 05_集成工具_Integrate_Tools/tools/llm_client.py）。

不做成跨 Agent import——每个 Agent 自己的 tools/ 保持自包含，是已确立的
惯例（PTA/OB 各自维护一份，不是共享一个外部包，见 OB 该文件自己的
docstring）。这份逻辑本身已经在 PTA/OB 上真实跑过多次、修过真实的 SSL
证书路径坑、429限流退避、pro模型切换后的超时坑，直接复用已验证过的
实现，不重新发明。
"""
