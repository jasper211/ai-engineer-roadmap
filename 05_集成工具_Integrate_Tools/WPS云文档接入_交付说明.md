...
**使用者必须已经是企业成员，并且对目标文档库有访问权限。**

这套接入用的是「用户授权」——Agent 能读到的范围**完全等于该使用者本人在 WPS 里能看到的范围**。
不在企业内的人，即使安装和授权全部成功，也读不到任何企业文档，而且失败信息很隐晦
（列团队库会返回 `400002059 用户不在企业内`）。

### 一分钟自检

让使用者在浏览器打开 https://www.kdocs.cn ，用他平时的账号登录：

| 看到什么 | 能不能装 |
|---|---|
| 左侧有「团队文档」，且能看到需要读的那些库 | ✅ 可以装 |
| 只有「我的云文档」，没有团队文档 | ❌ **先联系管理员加入企业并授予文档权限** |
| 能看到团队文档，但缺某几个库 | ⚠️ 装完也读不到那几个库，先让管理员补权限 |

> **注意**：账号可能同时有个人身份和企业身份。如果用手机号/微信登录后只看到个人空间，
> 找界面上的「切换企业/切换身份」切到公司组织，再看一次。
> 授权那一步登录的必须是**能看到团队文档的那个身份**。

### 企业外人员怎么办

平台不支持用应用身份绕开（实测 `--token-type app` 访问文件返回
`403 企业套餐权益不足，请开通商业高级版/旗舰版`）。所以只有两条路：
**加入企业**，或**由企业内成员导出数据**给他。
