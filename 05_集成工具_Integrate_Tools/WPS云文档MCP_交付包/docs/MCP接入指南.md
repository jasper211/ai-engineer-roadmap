...
| `wps_read_document` | 读 Word(.docx)/PPT(.pptx)/纯文本 的**正文**，支持关键词过滤与分段续读 |
...
1. **PDF 与旧版 .doc 读不到正文。** `.docx/.pptx` 和纯文本已支持（下载后用标准库解析），
   但 PDF 和二进制的 .doc 需要专门的解析库，与零依赖原则冲突，故不支持。
   实测很多合同同时存有 .docx 和 .pdf 两版，**优先找 .docx**。

2. **中文文件名简繁混用，搜索不做转换。** 实测同一文件夹里正文用繁体
   （顧問服務協議），附件却用简体（服务工作范围及验收标准）。`wps_search_files`
   是精确子串匹配，只搜一种写法会漏掉另一种。**中文关键词请简繁各搜一次**，
   搜不到时优先怀疑简繁问题，其次才是深度不够。
   这个坑也存在于正文：实测一份文件名是简体「服务工作范围及验收标准」的文档，
   正文标题却是繁体「服務工作範圍及驗收標準」，`wps_read_document` 的 `contains` 同样要简繁都试。

3. **表格按选区读。** `wps_read_sheet` 默认前 200 行 × 30 列，大表要分块读。
   超出范围的数据不会提示缺失，**心里要有数**。

4. **多维表格读取尚未用真实数据验证。** 目前接触到的库里没有 `.dbt` 文件，
   `wps_read_dbsheet` 的翻页游标字段名是按文档实现+多字段名容错，首次实跑可能需要微调。

5. **APPKEY 分发是个权衡。** 每个使用者的 MCP 配置里都要填 APPKEY，等于密钥在团队内扩散。
