...
校验：
  ...
  3. inforce 成分 vs L13（schema-aware L11 路由）：
     - 2023(pre-RBC)：L11 贡献 policy_count + contribution；L13=Σ(L8,9,10,12).metric + L11.*
     - 2024(RBC)：L11 贡献 scheme_count + contribution；L13=Σ(L8,9,10,12).metric + L11.*
     - current_estimate：Σ(L8..12) 直接
容差：保单数/人数/scheme 精确(≤0.05)；金额 1e-3 (HKD_thousand)。
"""
import os, sqlite3
DB=os.path.join(os.path.dirname(__file__),"..","data","annual_company_fact_layer_2023_2024.db")
...
print("\n=== 3. inforce 成分 vs L13（schema-aware L11 路由） ===")
print("   L13 总保单数/总保费 并入 L11(退休计划)：2023 L11→policy_count+contribution；2024 L11→scheme_count+contribution")
for year in (2024,2023):
    print(f"  -- {year} --")
    ...
    sbase=sum((sum_ins(year,t,"current_estimate") or 0) for t in ["L8","L9","L10","L11","L12"])
    s13=sum_ins(year,"L13","current_estimate")
    if s13 is not None:
        d=abs(sbase-s13); note="OK" if d<1e-3 else "<-- MISMATCH"
        print(f"     Σ(L8..12).current_estimate={sbase:,.3f}  L13={s13:,.3f} diff={d:.4f} {note}")
conn.close()
print("\nQA DONE")
