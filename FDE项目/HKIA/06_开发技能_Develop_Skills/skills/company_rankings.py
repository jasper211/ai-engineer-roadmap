...
# "非银"排名剔除的是"银行系"保险公司本身（母行是滙豐/中銀，几乎全部业务走
# 自家银行渠道），不是"每家公司剔除自己的银行渠道销售额"——用剔除后的公司
# 名单重排，但分母（市场份额的总量）仍然是全市场总量，不重新计算。这条名单
# 是对着"香港保险圈"同期排名反推验证出来的（剔除这两家、其余公司份额分母
# 不变，数字才对得上），不是从任何单一sheet字段能直接读出来的，是需要人工
# 维护的名单——以后如果哪家银行系保险公司改了名字/新增了一家，这里要跟着改。
BANK_AFFILIATED_INSURERS = {"滙豐人壽", "中銀人壽"}
