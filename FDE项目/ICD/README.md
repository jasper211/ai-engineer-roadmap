...
# 初始化 SQLite（幂等：建12张表 + 导入11家险企/21条数据源/11条错误代码；旧库自动做 pre-v4.bak 备份并幂等迁移）
...
# 解析最新成功抓取的快照（AIA JSON source_id=1 / CTF Life HTML source_id=5 / CLO HTML source_id=9 → fulfillment_ratio；Prudential General Insurance RBC PDF source_id=13（PRUGI）→ rbc_statement；需先 --fetch）
