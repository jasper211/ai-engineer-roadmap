...
# -*- coding: utf-8 -*-
"""
ICD · skills/pru_rbc_parser.py · Prudential RBC 披露声明 PDF 解析与标准化
（L3-ICD-03 解析层 · format=pdf 分支 · insurer=PRU）

职责边界：纯函数，把 Prudential RBC 披露声明 PDF 的原始字节解析为一条标准化
rbc_statement 记录（+ 可选原始风险分解 JSON）。不访问网络、不读库、不写库（入库由
tools/rbc_writer + skills/parse_disclosure 负责）。不执行 PDF 内嵌代码。

== 语义邻域（对齐任务书 T007 功能要求 3）==
只在明确的「Capital adequacy / Ratio of capital base to prescribed capital amount」
语义邻域内提取，绝不全页百分号正则猜数：
- report_year：来自披露时点 "31 December YYYY"（唯一且非空）。
- legal_entity_name_raw：来自 "Authorized insurer's name" 邻域的法律主体逐字原文
  （T007 Round 2：主体是核心归属，缺失即 STRUCTURE_MISMATCH）。
- solvency_ratio：来自 "Ratio of capital base to prescribed capital amount" 标签
  紧邻的百分比（如 "290%" → 2.90，原文 "290%"）。
...
- 结构漂移（缺 Capital adequacy 段落 / 缺 ratio 标签 / 报告年度缺失或不一致）→
  PruRbcParseError（STRUCTURE_MISMATCH）。
- 核心比率缺失或歧义（0 个或 >1 个不同候选百分比）→ PruRbcParseError（STRUCTURE_MISMATCH）。
...
class PruRbcParseError(Exception):
    """Prudential RBC PDF 结构不符/缺失/歧义，对应 STRUCTURE_MISMATCH 硬失败。"""
...
LEGAL_ENTITY_LABEL = "authorized insurer's name"
...
def _extract_legal_entity_name(full_text: str) -> str:
    """从 'Authorized insurer's name'（或 'Company Name:'）邻域提取法律主体逐字原文。

    优先逐行：标签独占一行时取下一非空行；再同行：'Authorized insurer's name: X'；
    最后回退 'Company Name:'（Statement of Compliance 段）。找不到即 PruRbcParseError
    （主体身份是核心归属，缺失不可安全归属）。
    """
    lines = [l.strip() for l in full_text.splitlines()]
    for i, line in enumerate(lines):
        nl = _norm(line).replace("\u2019", "'")
        if nl in (LEGAL_ENTITY_LABEL, LEGAL_ENTITY_LABEL + ":"):
            for j in range(i + 1, len(lines)):
                cand = lines[j].strip()
                if cand:
                    return _norm_keep_case(cand)
            break
    m = re.search(
        r"authorized\s+insurer['\u2019]?s\s+name\s*:?\s*([A-Za-z][^\n]*)",
        full_text, re.IGNORECASE,
    )
    if m and m.group(1).strip():
        return _norm_keep_case(m.group(1))
    m = re.search(r"company\s+name\s*:?\s*([A-Za-z][^\n]*)", full_text, re.IGNORECASE)
    if m and m.group(1).strip():
        return _norm_keep_case(m.group(1))
    raise PruRbcParseError("未找到 'Authorized insurer's name'（法律主体名称缺失，不可安全归属）")


def _extract_ratio(full_text: str, all_tables) -> Tuple[str, float]:
    """在 ratio 标签邻域提取唯一百分比。

    0 个 → 缺失（PruRbcParseError）；>1 个不同值 → 歧义（PruRbcParseError）。
    返回 (原始字符串, 小数比率)。
    """
    ...
    if not candidates:
        raise PruRbcParseError("偿付能力比率缺失（未找到 ratio 标签紧邻的百分比）")
    if len(candidates) > 1:
        raise PruRbcParseError(
...
def extract_rbc(pages: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    结构不符/缺失/歧义 → PruRbcParseError（STRUCTURE_MISMATCH）。
    """
    ...
    if SECTION_HEADING not in norm_text:
        raise PruRbcParseError("未找到 'Capital adequacy' 段落（结构漂移/非 RBC 披露）")
    ...
    if not years:
        raise PruRbcParseError("报告年度缺失（未找到 '31 December YYYY'）")
    if len(years) != 1:
        raise PruRbcParseError(f"报告年度不一致或歧义: {sorted(years)}")
...
def parse_pru_rbc(body: bytes) -> Dict[str, Any]:
    """解析 Prudential RBC PDF 字节 → 标准化结果（先提取文字层，再做语义邻域解析）。"""
