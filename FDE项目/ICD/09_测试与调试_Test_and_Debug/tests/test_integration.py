...
- SQLite 初始化：12 张表、12 家险企、24 条数据源、11 条错误代码种子；
...
def test_init_db_schema_and_seeds():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        check(c["data_source"] == 24, f"数据源 24 条（实际 {c['data_source']}）", f"数据源数错误：{c['data_source']}")
...
def test_init_db_idempotent():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        check(c["data_source"] == 24, f"二次初始化数据源仍 24（实际 {c['data_source']}）", f"数据源重复：{c['data_source']}")
...
def test_cli_status_db_absent_and_present():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        check(data is not None and data["source_count"] == 24, "source_count=24", f"source_count 错误：{data}")
