...
- SQLite 初始化：12 张表、12 家险企、22 条数据源、11 条错误代码种子；
...
def test_init_db_schema_and_seeds():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        c = summary["counts"]
        check(c["insurer"] == 12, f"险企 12 家（实际 {c['insurer']}）", f"险企数错误：{c['insurer']}")
        check(c["data_source"] == 22, f"数据源 22 条（实际 {c['data_source']}）", f"数据源数错误：{c['data_source']}")
...
def test_init_db_idempotent():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        c = summary2["counts"]
        check(c["insurer"] == 12, f"二次初始化险企仍 12（实际 {c['insurer']}）", f"险企重复：{c['insurer']}")
        check(c["data_source"] == 22, f"二次初始化数据源仍 22（实际 {c['data_source']}）", f"数据源重复：{c['data_source']}")
...
def test_cli_status_db_absent_and_present():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        check(data is not None and data["insurer_count"] == 12, "insurer_count=12", f"insurer_count 错误：{data}")
        check(data is not None and data["source_count"] == 22, "source_count=22", f"source_count 错误：{data}")
